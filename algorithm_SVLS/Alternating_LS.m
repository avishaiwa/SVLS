 ...
 %           (kron(inv(U12), inv(U22)) * big_b(n1*k_r+1:end)')']; 
        new_c = [ ...
         ...
         new_c = ( ...
           ...
           new_c(n1*k_r+1:end)'))' ./ ...
           sqrt(sum(A_part2.^2));
 %       new_c_fast3 = (A_part2' * new_c_fast2')' ./  sqrt(sum(A_part2.^2));
 %       new_c_fast4 = [kron(sparse(Diag11), sparse(Diag21))' * new_c_fast2(1:n1*k_r)'  ...
 %          + kron(sparse(Diag12), sparse(Diag22))'  * new_c_fast2(n1*k_r+1:end)']' ./ ...
 %          sqrt(sum(A_part2.^2));
       
 %       (kron(inv(U11), inv(U21)) * big_b(1:n1*k_r)')'  ...
 %           (kron(inv(U12), inv(U22)) * big_b(n1*k_r+1:end)')']; 
 %       new_c_fast = (A_part2' * new_c_fast')' ./  sqrt(sum(A_part2.^2));
