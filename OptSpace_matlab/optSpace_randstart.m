function [X S Y dist] =optSpace_randstart(M_E,N_hat,r,niter,tol)
% This version of optSpace start with random matri insted the SVD(N_hay)
...
dist(1) = norm( (N_hat - X*S*Y').*E ,'fro')/sqrt(nnz(E) )  ;
fprintf(1,'0\t\t%e\n',dist(1) ) ;

for i = 1:niter
% Compute the Gradient 
	[W Z] = gradF_t(X,Y,S,N_hat,E,m0,rho);

% Line search for the optimum jump length	
	t = getoptT(X,W,Y,Z,S,N_hat,E,m0,rho) ;
    X1=X; Y1=Y; S1=S;
	X = X + t*W;Y = Y + t*Z;S = getoptS(X,Y,N_hat,E) ;
	
% Compute the distortion	
	dist(i+1) = norm( (X1*S1*Y1' - X*S*Y').*E,'fro' );%/sqrt(nnz(E));
	%fprintf(1,'%d\t\t%e\n',i,dist(i+1) ) ;
	if( dist(i+1) < tol )
		break ;
	end
...
function out = F_t(X,Y,S,N_hat,E,m0,rho)
[n r] = size(X) ;

out1 = sum( sum( ( (X*S*Y' - N_hat).*E ).^2 ) )/2 ;
...
function [W Z] = gradF_t(X,Y,S,N_hat,E,m0,rho)
[n r] = size(X);
[m r] = size(Y);
...
Qx = X'* ( (N_hat - XSY).*E )*YS /n;
Qy = Y'* ( (N_hat - XSY).*E )'*XS /m;

W = ( (XSY - N_hat).*E )*YS + X*Qx + rho*Gp(X,m0,r);
Z = ( (XSY - N_hat).*E )'*XS + Y*Qy + rho*Gp(Y,m0,r);
...
function out = getoptT(X,W,Y,Z,S,N_hat,E,m0,rho)
norm2WZ = norm(W,'fro')^2 + norm(Z,'fro')^2;
f(1) = F_t(X, Y,S,N_hat,E,m0,rho) ;

t = -1e-1 ;
for i = 1:20
        f(i+1) = F_t(X+t*W,Y+t*Z,S,N_hat,E,m0,rho) ;
