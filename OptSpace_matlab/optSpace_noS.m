function [X S Y dist] =optSpace_noS(M_E,r,niter,tol)
%optSpace without finding optimal S
...
dist(1) = norm( (M_E - X*Y').*E ,'fro')/sqrt(nnz(E) )  ;
%fprintf(1,'0\t\t%e\n',dist(1) ) ;

for i = 1:niter

% Compute the Gradient 
	[W Z] = gradF_t(X,Y,M_E,E,m0,rho);

% Line search for the optimum jump length	
	t = getoptT(X,W,Y,Z,M_E,E,m0,rho) ;
	 X1 = X; Y1 = Y; 
	X = X + t*W;Y = Y + t*Z;
	
% Compute the distortion	
	dist(i+1) = norm( (X1*Y1' - X*Y'),'fro' );%/sqrt(nnz(E));
%	fprintf(1,'%d\t\t%e\n',i,dist(i+1) ) ;
	if( dist(i+1) < tol )
		break ;
	end
...
function out = F_t(X,Y,M_E,E,m0,rho)
[n r] = size(X) ;

out1 = sum( sum( ( (X*Y' - M_E).*E ).^2 ) )/2 ;
...
function [W Z] = gradF_t(X,Y,M_E,E,m0,rho)
[n r] = size(X);
[m r] = size(Y);
...
W = ( (XY - M_E).*E )*Y;% + X*Qx + rho*Gp(X,m0,r);
Z = ( (XY - M_E).*E )'*X;% + Y*Qy + rho*Gp(Y,m0,r);
...
function out = getoptT(X,W,Y,Z,M_E,E,m0,rho)
norm2WZ = norm(W,'fro')^2 + norm(Z,'fro')^2;
f(1) = F_t(X, Y,M_E,E,m0,rho) ;

t = -1e-1 ;
for i = 1:20
        f(i+1) = F_t(X+t*W,Y+t*Z,M_E,E,m0,rho) ;
